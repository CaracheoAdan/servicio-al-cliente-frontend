...
export function ReportsPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in-up font-body">
      ...
      {loading ? (
        <div className="flex justify-center items-center h-64 bg-white rounded-[28px] shadow-card-base border border-[#E3E9E6]">
          <div className="text-center">
             <div className="inline-block animate-spin w-8 h-8 border-4 border-[#15803D] border-t-transparent rounded-full mb-4"></div>
             <p className="font-display font-bold text-[#0F1B17]">Calculando estadísticas...</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-[28px] shadow-card-base border border-[#E3E9E6] flex items-center space-x-4 card-glow transition-all">
              <div className="bg-[#F0FDF4] p-4 rounded-2xl">
                <TrendingUp className="w-8 h-8 text-[#15803D]" />
              </div>
              <div>
                <p className="text-xs font-display font-bold text-[#6B7B76] uppercase tracking-wide">Eficiencia OEE</p>
                <p className="text-3xl font-mono font-bold text-[#0F1B17] mt-1">92<span className="text-lg text-[#9CA8A3]">%</span></p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-[28px] shadow-card-base border border-[#E3E9E6] flex items-center space-x-4 card-glow transition-all">
              ...
              <div>
                <p className="text-xs font-display font-bold text-[#6B7B76] uppercase tracking-wide">Órdenes Procesadas</p>
                <p className="text-3xl font-mono font-bold text-[#0F1B17] mt-1">{transportData.length} <span className="text-lg text-[#9CA8A3]">órdenes</span></p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-[28px] shadow-card-base border border-[#E3E9E6] flex items-center space-x-4 card-glow transition-all">
              ...
              <div>
                <p className="text-xs font-display font-bold text-[#6B7B76] uppercase tracking-wide">Cumplimiento Global</p>
                <p className="text-3xl font-mono font-bold text-[#0F1B17] mt-1">
                  {fulfillmentData.length > 0 ? Math.round(fulfillmentData.reduce((acc, curr) => acc + curr.fulfillment, 0) / fulfillmentData.length) : 0}
                  <span className="text-lg text-[#9CA8A3]">%</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white p-8 rounded-[28px] shadow-card-base border border-[#E3E9E6]">
              <div className="flex items-center mb-6">
                <div className="w-2 h-6 bg-[#15803D] rounded-full mr-3"></div>
                <h4 className="text-lg font-display font-bold text-[#0F1B17]">Salida de Camiones (shipping_date)</h4>
              </div>
              <p className="text-xs font-mono text-[#9CA8A3] mb-6">Regla: Salida 11:00 AM. Tolerancia hasta 11:30 AM.</p>
              
              <div className="h-80 w-full relative">
                {transportData.length === 0 ? (
                  <div className="absolute inset-0 flex items-center justify-center font-display font-bold text-[#9CA8A3]">No hay órdenes con fecha de salida registrada.</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={transportData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E3E9E6" />
                      <XAxis dataKey="order" tick={{fill: '#6B7B76', fontSize: 12, fontFamily: 'IBM Plex Mono'}} axisLine={false} tickLine={false} />
                      <YAxis domain={[9, 13]} tickFormatter={formatTimeAxis} tick={{fill: '#6B7B76', fontSize: 12, fontFamily: 'IBM Plex Mono'}} axisLine={false} tickLine={false} />
                      <Tooltip 
                        formatter={(value: number, name: string, props: any) => [props.payload.label, 'Salida']}
                        contentStyle={{ borderRadius: '16px', border: '1px solid #E3E9E6', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', fontFamily: 'IBM Plex Mono' }}
                      />
                      <ReferenceArea y1={11.0} y2={11.5} fill="#15803D" fillOpacity={0.1} />
                      <ReferenceLine y={11.0} stroke="#15803D" strokeDasharray="4 4" label={{ position: 'top', value: 'Meta (11:00)', fill: '#15803D', fontSize: 12, fontFamily: 'IBM Plex Mono' }} />
                      ...
                      <Line 
                        ...
                        stroke="#15803D" 
                        strokeWidth={4}
                        dot={{ fill: '#15803D', strokeWidth: 2, r: 6, stroke: '#ffffff' }}
                        activeDot={{ r: 8, strokeWidth: 0, fill: '#116932' }} 
                      />
                    </LineChart>
                  </ResponsiveContainer>
                )}
            ...
            <div className="bg-white p-8 rounded-[28px] shadow-card-base border border-[#E3E9E6]">
              <div className="flex items-center mb-6">
                <div className="w-2 h-6 bg-[#15803D] rounded-full mr-3"></div>
                <h4 className="text-lg font-display font-bold text-[#0F1B17]">Nivel de Cumplimiento</h4>
              </div>
              <p className="text-xs font-mono text-[#9CA8A3] mb-6">Fórmula: (Entregado / Pedido) * 100%.</p>
              
              <div className="h-80 w-full relative">
                {fulfillmentData.length === 0 ? (
                  <div className="absolute inset-0 flex items-center justify-center font-display font-bold text-[#9CA8A3]">No hay productos en las órdenes actuales.</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={fulfillmentData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E3E9E6" />
                      <XAxis dataKey="order" tick={{fill: '#6B7B76', fontSize: 12, fontFamily: 'IBM Plex Mono'}} axisLine={false} tickLine={false} />
                      <YAxis domain={[0, 100]} tickFormatter={(val) => `${val}%`} tick={{fill: '#6B7B76', fontSize: 12, fontFamily: 'IBM Plex Mono'}} axisLine={false} tickLine={false} />
                      <Tooltip 
                        formatter={(val) => [`${val}%`, 'Cumplimiento']}
                        contentStyle={{ borderRadius: '16px', border: '1px solid #E3E9E6', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', fontWeight: 'bold', fontFamily: 'IBM Plex Mono' }}
                        cursor={{fill: '#F3F6F4'}}
                      />
                      <Bar 
                        dataKey="fulfillment" 
